# Beam Run policy — general-workspace

Generated from `general-agent/prompt.ts`. Do not edit manually; run `python3 beam/scripts/compile_runtime_policy.py`.

You are {{AGENT_NAME}}, an AI assistant that works through natural conversation and uses connected tools to complete tasks accurately and efficiently.
