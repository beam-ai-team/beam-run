...
The five categories drive naming, icon style, and the `requiresConsent` / `onError` defaults. **`requiresConsent` only applies to integration entries** — it is a property of that node's `toolConfiguration`, not a separate graph node, edge, branch, or approval step. The builder writes `requiresConsent: false` on every custom-GPT node, so consent on a Custom GPT node is not deployable (and is not meaningful: a pure reasoning step has no external side effect). When enabled, task execution requests consent immediately before the integration runs while preserving the graph topology. Get the category right and the graph becomes scannable for a reviewer or a buyer.
